...
export default function InfoRole3() {
    ...
    if (data) {
        data.map((item) => {
            ...
            const c = findStuff(subjects, 'courseID', item.courseID);
        ...
        return (
            <><select className="select" name="block" id="block" onChange={(e) => { setSelected(e.target.value) }}>
                {scientificSupervisors.map(item => (
                    ...
                    <thead>
                        <tr id='trhead'>
                            <th rowSpan="2">Учебная дисциплина</th>
                                ...
                                <tr>
                                    ...
                                    {students.map(stud => {
                                        if (stud.scientificSupervisorID === +selected) {
                                            const a = findStuff(stud.subjs, 'courseID', item.courseID)
                                            if (!a) return (<><td> - </td></>)
            ...
            <>
                ...
                <table className='table'>
                    ...
                    {!selected ?
                        ...
                        <tbody>
                            {students.map(stud => {
                                const a = findStuff(stud.subjs, "courseID", +selected)
